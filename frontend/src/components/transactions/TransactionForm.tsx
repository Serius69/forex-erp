...
const validationSchema = yup.object({
  ...
  customerName: yup.string().when('customerId', {
    is: null,
    then: yup.string().required('Nombre del cliente requerido'),
  }),
  documentNumber: yup.string().when('customerId', {
    is: null,
    then: yup.string().required('Número de documento requerido'),
  }),
...
const TransactionForm: React.FC<TransactionFormProps> = ({ open, onClose, onSuccess }) => {
  ...
  const handleNext = () => {
    ...
    const errors = fieldsToValidate.reduce((acc, field) => {
      const error = validationSchema.fields[field as keyof typeof validationSchema.fields];
      try {
        error?.validateSync(formik.values[field as keyof typeof formik.values]);
      } catch (e: any) {
        acc[field] = e.message;
      }
      return acc;
