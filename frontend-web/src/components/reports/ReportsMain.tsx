...
const ReportsMain: React.FC = () => {
  ...
  // Sucursal para el libro diario: la seleccionada globalmente (ADMIN) o la propia.
  const dailyLogBranchId          = branchId ?? user?.branch_id ?? user?.branch?.id;

  const handleDownload = async (
    endpoint: string,
    filename: string,
    params:   object = {},
    fmt:      'excel' | 'pdf' | 'csv' = 'excel'
  ) => {
  ...
  const ReportCard = ({
    title, description, icon, onExcel, onPdf, onCsv, loadingKey,
  }: {
    title:       string;
    description: string;
      ...
      <CardActions sx={{ p: 2, gap: 1 }}>
        ...
        {onCsv && (
          <Button size="small" variant="outlined" color="success"
            startIcon={loading === `${loadingKey}-csv`
              ? <CircularProgress size={14} /> : <TableChart />}
            onClick={onCsv} disabled={!!loading}>
            CSV
          </Button>
        )}
      </CardActions>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6} md={4}>
              <ReportCard
                ...
                onCsv={() => handleDownload(
                  '/reports/asfi/rte/download-csv/',
                  `RTE_${year}_${month.toString().padStart(2,'0')}`,
                  { year, month }, 'csv'
                )}
              />
