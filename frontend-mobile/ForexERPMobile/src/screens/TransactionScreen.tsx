// TransactionScreen.tsx - Pantalla para
...
import { offlineQueue, isNetworkError } from '../services/offlineQueue';
...
export default function TransactionScreen() {
  const { pin } = useAuth();
  const { pendingCount, isSyncing, sync, refreshCount } = useOfflineSync();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isNetworkError(err)) {
        await offlineQueue.enqueue(payload, pin);
        await refreshCount();
        Alert.alert(
          'Sin conexión',
          'La transacción fue guardada localmente y se sincronizará automáticamente cuando haya conexión.',
          [{ text: 'OK' }],
        );
        resetForm();
      } else {
          ...
          <View style={styles.offlineBanner}>
            <View style={styles.offlineBannerInfo}>
              ...
              <Text style={styles.offlineBannerText}>
                {pendingCount} transacción{pendingCount !== 1 ? 'es' : ''} pendiente{pendingCount !== 1 ? 's' : ''}
              </Text>
            </View>
            <TouchableOpacity
              ...
              onPress={() =>
                sync().then(r => {
                  if (r.ok > 0) {
                    Alert.alert(
                      'Sincronizado',
                      `${r.ok} transacción${r.ok !== 1 ? 'es' : ''} enviada${r.ok !== 1 ? 's' : ''} correctamente.`,
                    );
                  }
                })
              }
              disabled={isSyncing}
            >
              <Text style={styles.syncBtnText}>{isSyncing ? 'Enviando...' : '↻ Sincronizar'}</Text>
            </TouchableOpacity>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  offlineBanner: {
    backgroundColor: '#FFF3CD', borderBottomWidth: 1, borderBottomColor: '#FFECB5',
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingHorizontal: 16, paddingVertical: 10,
  },
  offlineBannerInfo: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  offlineBannerIcon: { fontSize: 16, marginRight: 6 },
  offlineBannerText: { fontSize: 13, fontWeight: '600', color: '#856404', flex: 1 },
  syncBtn: { backgroundColor: '#1E3A5F', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 },
