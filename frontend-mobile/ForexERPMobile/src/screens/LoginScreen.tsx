// LoginScreen.tsx - Pantalla de inicio de sesión para la aplicación móvil Forex ERP
...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      await login({ username: username.trim(), password, pin: pin.trim() });
    } catch (err: any) {
      Alert.alert(
        'Error de acceso',
    ...
    >
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <View style={styles.field}>
            <Text style={styles.label}>PIN de operación (opcional)</Text>
            <TextInput
              style={styles.input}
              placeholder="PIN para transacciones con supervisor"
              placeholderTextColor="#AAB4BE"
              value={pin}
              onChangeText={setPin}
            ...
            <Text style={styles.hint}>
              Se guarda de forma segura en el dispositivo y se usa para autorizar
              transacciones que requieren aprobación.
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 11, color: '#9AA6B2', marginTop: 6, lineHeight: 15 },
  input: {
