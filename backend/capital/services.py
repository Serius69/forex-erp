...
class CashBOBService:
    ...
    def deducir_bob(branch, amount: Decimal, save: bool = True) -> dict:
        ...
        with db_tx.atomic():
            ...
            # ─ Caja chica (smallest first: 10, 20, 50, 100, 200) ─────────────
            denoms_cc = [
                ('caja_chica_10', 10), ('caja_chica_20', 20),
                ('caja_chica_50', 50), ('caja_chica_100', 100),
                ('caja_chica_200', 200),
            ]
            remaining, ops = CashBOBService._deduct_from_group(
                cash, denoms_cc, remaining, ops, 'caja_chica'
            )

            # ─ Sueltos (smallest first: 10, 20) ──────────────────────────────
            if remaining > 0:
                denoms_s = [('sueltos_10', 10), ('sueltos_20', 20)]
                remaining, ops = CashBOBService._deduct_from_group(
                    cash, denoms_s, remaining, ops, 'sueltos'
                )

            # ─ Fuertes (smallest first: 50, 100, 200) ────────────────────────
            if remaining > 0:
                denoms_f = [
                    ('fuertes_50', 50), ('fuertes_100', 100), ('fuertes_200', 200),
                ]
                remaining, ops = CashBOBService._deduct_from_group(
                    cash, denoms_f, remaining, ops, 'fuertes'
                )
    ...
    @staticmethod
    def _deduct_from_group(cash, denoms: list, remaining: int,
                            ops: list, group: str):
        """Greedily deduct ``remaining`` BOB from denomination group (in-place)."""
        for field, value in denoms:
            if remaining <= 0:
                break
            available = getattr(cash, field)
            use = min(available, remaining // value)
            if use > 0:
...
def _serialize_resultado(r: dict) -> dict:
    """Convierte Decimals a str para serialización JSON."""
    for k in ('capital_neto', 'total_activos', 'total_pasivos'):
