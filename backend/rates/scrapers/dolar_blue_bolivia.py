...
def scrape_parallel_rate() -> dict:
    ...
    if not result["mid"] or not result["buy"]:
        ...
        if valid:
            ...
            # Mediana solo si no podemos derivar mid de buy+sell (promedio
            # exacto, calculado más abajo) — evita sesgar mid al máximo
            # cuando los selectores CSS ya encontraron compra y venta.
            if not result["mid"] and not (result["buy"] and result["sell"]):
