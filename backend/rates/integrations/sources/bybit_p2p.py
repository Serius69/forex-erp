"""
Bybit P2P — wrapper para la capa integrations/.
Reutiliza la lógica de fetchers/p2p_exchanges.py y adapta a NormalizedRate.
"""
...
log = logging.getLogger('kapitalya.integrations.bybit_p2p')


class BybitP2PIntFetcher(AbstractRateFetcher):
    id_fuente       = 'bybit_p2p'
    tipo_fuente     = 'P2P'
    pares_soportados = [('USD', 'BOB')]

    def fetch(self) -> list[NormalizedRate]:
        from rates.fetchers.p2p_exchanges import BybitP2PFetcher
        legacy = BybitP2PFetcher()
