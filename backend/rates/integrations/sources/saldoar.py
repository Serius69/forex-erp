"""
SaldoAR — wrapper para la capa integrations/.
"""
...
log = logging.getLogger('kapitalya.integrations.saldoar')


class SaldoARIntFetcher(AbstractRateFetcher):
    id_fuente       = 'saldoar'
    tipo_fuente     = 'AGREGADOR'
    pares_soportados = [('ARS', 'BOB')]

    def fetch(self) -> list[NormalizedRate]:
        from rates.fetchers.saldoar_fetcher import SaldoARFetcher
        legacy = SaldoARFetcher()
